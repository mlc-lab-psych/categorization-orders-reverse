...
app.get('/get-data', (req, res) => {
    ...
    async function processCountData() {
        ...
        try {
            const snapshot = await get(child(dbRef, 'count-reverse'));
            if (snapshot.exists()) {
                ...
                const updates = {};
                updates[`count-reverse/${key}`] = countData[key] + 1
...
app.post('/submit-results', async (req, res) => {
    ...
    try{
        await set(ref(database, 'users-new-reverse/' + randomId), cleanedData)
            .then(() => {
